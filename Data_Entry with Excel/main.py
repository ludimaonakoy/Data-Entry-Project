...
def enter_data():
    ...
    if accepted == "accepted":
        ...
        if firstname and lastname:
            ...
            #python list
            # which only add the header to the excel file
            heading = ["First_Name","Last_Name","Title","Age","Nationality",
                       "# Courses","# Semester", "Registration_status"]
            #the append : attach the heading to the excel sheet
            sheet.append(heading)
            workbook = openpyxl.load_workbook("data.xlsx")
            # to get the active sheet
            sheet = workbook.active
            #create the python will only contain the variable we have we put them in the list
            sheet.append([firstname,lastname, title,age,nationality,numbercourse,numsemesters,registration])
            # we have t0 save
